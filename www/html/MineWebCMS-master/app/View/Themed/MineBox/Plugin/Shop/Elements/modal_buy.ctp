  ...
  <p class="name"><b>{LANG-SHOP__ITEM_NAME} :</b> {ITEM_NAME}</p>
  <div class="description">
    <p class="description-title"><b>{LANG-SHOP__ITEM_DESCRIPTION} :</b> {ITEM_DESCRIPTION}</p>      
  </div>
  [IF AFFICH_SERVER]
    <p><b>{LANG-SERVER__TITLE} :</b> {ITEM_SERVERS}</p>
  [/IF AFFICH_SERVER]
  <p class="price"><b>{LANG-SHOP__ITEM_PRICE} :</b> {ITEM_PRICE} {SITE_MONEY}</p>
...
<div class="modal-footer">
  <div class="pull-left">
    <button class="total">{LANG-SHOP__ITEM_TOTAL} : <span id="total-price">{ITEM_PRICE}</span>  {SITE_MONEY}</button>
  </div>
  [IF MULTIPLE_BUY]
    <div class="col-md-3">
