		...
		<div class="panel-body">
			...
			<ul class="list-category" role="tablist">
				...
				foreach ($search_categories as $k => $v) {
				  $i++;
				?>
					<li style="margin: 5px"><a href="<?= $this->Html->url(array('controller' => 'c/'.$v['Category']['id'], 'plugin' => 'shop')) ?>" class="<?= (isset($category) AND $v['Category']['id'] == $category OR !isset($category) AND $i == 1) ? ' active' : ''; ?>"><?= before_display($v['Category']['name']) ?></a></li>
				<?php } ?>
			</ul>	
			<hr>
			<?php if(!$isConnected) { ?>
			...
			<?php } else { ?>
			<a href="#" class="btn btn-warning btn-block btn-sm text-uppercase"><?= $Lang->get('SHOP__MONEY_CURRENTLY') ?>: <?= $money ?> <?= $Configuration->getMoneyName(); ?></a>
