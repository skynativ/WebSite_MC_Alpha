<div class="col-md-8">
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      <?= $news['News']['content'] ?>
      <hr>
      <?= $Lang->get('GLOBAL__BY') ?> <?= $news['News']['author'] ?> <img src="<?= $this->Html->url(array('controller' => 'API', 'action' => 'get_head_skin', 'plugin' => false, $news['News']['author'], 16)) ?>" title="<?= $news['News']['author'] ?>">
      <span class="pull-right">
          <button id="<?= $news['News']['id'] ?>" type="button" class="like btn btn-sm btn-success<?= ($news['News']['liked']) ? ' active' : '' ?>"<?= (!$Permissions->can('LIKE_NEWS')) ? ' disabled' : '' ?>><?= $news['News']['count_likes'] ?> <i class="fa fa-thumbs-up"></i></button>
      </span>
    </div>
  </div>
  <!-- Commentaires -->
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      ...
      <?php foreach ($news['Comment'] as $k => $v) { ?>
        <div id="comment-<?= $v['id']?>">
          <h4>
            <img src="<?= $this->Html->url(array('controller' => 'API', 'action' => 'get_head_skin', 'plugin' => false, $v['author'], 32)) ?>" title="<?= $v['author'] ?>" alt=""> <?= $v['author'] ?>
...
<script type="text/javascript">
    ...
    function addcomment(data) {
      var d = new Date();
      var comment = '<h4><img src="<?= $this->Html->url(array('controller' => 'API', 'action' => 'get_head_skin', 'plugin' => false, $user['pseudo'], 32)) ?>" alt=""> <?= $user['pseudo']; ?><small>, à '+ d.getHours()+'h'+d.getMinutes()+'</small></h4><blockquote>'+data['content']+'</blockquote>';
